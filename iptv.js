...
// === Multi-Source Support (UDPTV / TheTVApp) ===
const sourceMap = {
...
let currentSource = localStorage.getItem("iptvSource") || "UDPTV";
let m3uUrl = sourceMap[currentSource];

// Create dropdown
const sourceSelector = document.createElement("select");
sourceSelector.style.position = "fixed";
sourceSelector.style.top = "10px";
sourceSelector.style.left = "10px";
sourceSelector.style.zIndex = "9999";
...
sourceSelector.addEventListener("change", () => {
  ...
  localStorage.setItem("iptvSource", currentSource);
  m3uUrl = sourceMap[currentSource];
  fetchAndBuildChannels();
});



// m3uUrl is now selected dynamically from dropdown
let channels = [];
let currentIndex = 0;
