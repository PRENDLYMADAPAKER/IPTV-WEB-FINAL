...
// ✅ Smart freeze detection and reconnect
let lastFreezeTime = video.currentTime;
let freezeAttempts = 0;

setInterval(() => {
  if (!video.paused && !video.ended && video.readyState >= 2) {
    if (video.currentTime === lastFreezeTime) {
      freezeAttempts++;
      if (freezeAttempts >= 3) {
        console.warn("📺 Freeze detected — reconnecting stream...");
